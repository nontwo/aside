...
The preserved extension can operate on real ChatGPT and Claude pages, but the course replication package uses only the synthetic data for privacy and reproducibility. The static demo does not run the extension and never contacts a provider.
...
The extension depends on provider page structure, which can change, and each provider is read through its own adapter so a change on one does not affect the other. The submission therefore separates the reproducible static demo from the browser extension artifact. The static app demonstrates the text-as-data logic without requiring live provider access.
